...
#Reads in data from the working directory (Option 1)
unzip(zipfile = "exdata_data_NEI_data.zip", list = TRUE)
...
if(!file.exists(DaTZipfile)) {
  download.file(Data_url, 
                destfile = DaTZipfile, 
                method = "curl")
  unzip(zipfile = "exdata_data_NEI_data.zip", list = TRUE, exdir = ".")
}
...
## Plotting
barplot((tol.emissions$Emissions)/10^3
        , names = tol.emissions$year
        , xlab = "Years", ylab = "PM2.5 Emissions (in 10^3 Tons)"
